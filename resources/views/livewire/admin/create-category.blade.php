<div class="container"> 
    ...
    <x-jet-form-section submit="save" class="mb-6">
        ...
        <x-slot name="form">
            ...
            <div class="justify-between col-span-6 sm:col-span-4">
                <x-jet-label>
                    Imagen
                </x-jet-label>

                <input wire:model="createForm.image" accept="image/*" type="file" class="flex mt-1" name="" id="{{$rand}}">
                    ...
                    @foreach ($categories as $category)
                        <tr>
                            <td class="py-2">
                                ...
                                <a href="{{route('admin.categories.show', $category)}}" class="underline {{-- uppercase --}} hover:text-blue-600">
                                    {{$category->name}}
                                </a>
                            </td>
                            <td class="py-2">
                                <div class="flex justify-between divide-x divide-gray-300">
                                    <a class="pr-2 cursor-pointer hover:text-blue-600" wire:click="edit('{{$category->slug}}')">Editar</a>
                                    <a class="pl-2 cursor-pointer hover:text-red-600" wire:click="$emit('deleteCategory', '{{$category->slug}}')">Eliminar</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
    ...
    <x-jet-dialog-modal wire:model="editForm.open">
        ...
        <x-slot name="content">
            ...
            <div class="space-y-3">
                ...
                <div>
                    @if ($editImage)
                        <img class="object-cover object-center w-full h-full" src="{{$editImage->temporaryUrl()}}" alt="">
                    @else
                        <img class="object-cover object-center w-full h-full" src="{{Storage::url($editForm['image'])}}" alt="">
                    @endif
                </div>
